...
let moneyData =
  localStorage.getItem("moneyInfo") === null
    ? [
        {
          totalMoney: Number(window.prompt("Qual valor deseja Administrar ?")),
          items: new Array(),
        },
      ]
    : JSON.parse(localStorage.getItem("moneyInfo"));
...
function createElementsAndInsertIntoPage(item, index) {
  ...
  h6.textContent = `${item.ammount} - ${item.name} - R$${item.value} - (R$ ${
    item.value * item.ammount
  })`;
  div.appendChild(h6);
...
function updateValuesFromPage() {
  let sumAllValuesDimi = () =>
    items.reduce((acc, item) => acc + item.value * item.ammount, 0);
...
addButton.onclick = () => {
  ...
  if (!newProductName.trim() || !newProductValue || newProductValue >= totalMoney) {
